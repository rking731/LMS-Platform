    ...
    setChapters(prevChapters =>
      prevChapters.map(chapter => {
        if (chapter.chapterId === currentChapterId) {
          ...
          // return a new chapter object with updated lecture list
          return {
            ...chapter,
            chapterContent: [...chapter.chapterContent, newLecture],
          };
        }
        return chapter; // unchanged chapter
      })
    );
...
useEffect(()=>{
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:pb-0 md:p-8 p-4 pt-8 pb-0'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-black'>
          <div className='flex flex-col gap-1'>
            <p>Course Title</p>
             <input onChange={e => setCourseTitle(e.target.value)} value={courseTitle} type="text" placeholder='Type here' 
             className='outline-none md:py-2.5 py-2 px-3 rounded border bg-white border-gray-500' required />
          </div>
          <div className='flex flex-col gap-1'>
            <p>Course Description</p>
            <div className='border-gray-500 border' ref={editorRef}></div>
          </div>
          ...
          <div className='flex flex-col gap-1'>
            ...
            <input onChange={e => setCoursePrice(e.target.value)} value={coursePrice} type="number" placeholder='0' 
             className='outline-none md:py-2.5 py-2 px-3 w-28 rounded border bg-white border-gray-500' required />
          </div>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
           ...
           <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
             ...
             <img src={image ? URL.createObjectURL(image) : ''} className='max-h-10 bg-white' alt="" />
           </label>
          </div>
         </div>
    ...
    {!chapter.collapsed && (
      <div className='p-4'>
        ...
        >
          ...
          <div className='flex justify-center items-center bg-blue-500 text-white p-2 rounded-lg cursor-pointer' onClick={() => handleChapter('add')}>+ Add Chapter</div>
          ...
          {showPopup && (
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
              ...
              <button onClick={addLecture} type='button' className='w-full bg-blue-600 text-white px-4 py-2 rounded'>Add</button>
         ...
         <button type='sunmit' className='bg-black text-white w-max py-2.5 px-8 rounded my-4'>ADD</button>
        </form>
